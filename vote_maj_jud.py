...
def save_votes(user_id, vote_key,votation_id,vote_array):
    ...
    option 2 has 5 and option 3 has 6. 
    """
    print(f"save_votes{vote_bo.save_votes(user_id, vote_key, votation_id, vote_array)}")
...
def count_votes_by_option(votation_id, option_id):
    ...
    print(f"count_votes_by_option jud_array {jud_array}")
    for j in range(len(jud_array)):
        ...
        ar.append( n )
    print(f"count_votes_by_option ar{ar}")
    return ar

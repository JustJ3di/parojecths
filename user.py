...
def load_latest_user_id():
    u = db.session.query(func.max(VotingUser.user_id)).first()
    return u[0]
...
def load_user_by_email(email):
    """Returns a user_dto object or None"""
    u = db.session.query(VotingUser).filter(VotingUser.email == email).first()
    if auth.ADD_UNKNOWN_USER:
        if u == None:
            u = VotingUser(email=email, pass_word='no password')
            db.session.add(u)
            db.session.commit()
    return u
